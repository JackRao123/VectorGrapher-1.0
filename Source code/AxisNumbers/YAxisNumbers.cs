...
using UnityEngine;
public class YAxisNumbers : MonoBehaviour
{
    ...
    public void spawnAxisNumbersAndDotsInRange()
    {
        ...
        float renderDistance = Mathf.Sqrt(Camera.main.transform.position.x * Camera.main.transform.position.x + Camera.main.transform.position.y * Camera.main.transform.position.y) / Mathf.Tan(renderAngle);
        lowerRange = Camera.main.transform.position.z - renderDistance;
        upperRange = Camera.main.transform.position.z + renderDistance;

        float cameraDistFromYAxis = Mathf.Sqrt(Camera.main.transform.position.x * Camera.main.transform.position.x + Camera.main.transform.position.y * Camera.main.transform.position.y);
      ...
      // print($"Increment = {increment }\n, LowerRange = {lowerRange}\n, UpperRange = {upperRange}\n, LowerRangeNumber = {lowerRangeNumber}\n, UpperRangeNumber = {upperRangeNumber}");
        for (float i = lowerRangeNumber; i <= upperRangeNumber; i = i + increment)
        {
            ...
            newAxisNumbers.Add(createAxisNumber((displayNumber).ToString(), new Vector3(50 * increment, 0, i * 100)));
            newAxisDots.Add(createAxisDot(new Vector3(0, 0, i * 100)));
    ...
    public void adjustAxisNumbersToFaceCamera() //adjusts the axis numbers to face the camera
    {
        ...
        {
            float newScale = 0.001f * Mathf.Sqrt(Camera.main.transform.position.y * Camera.main.transform.position.y + Camera.main.transform.position.x * Camera.main.transform.position.x);
            float cameraY = Camera.main.transform.position.z;
    ...
    public void adjustAxisDotSize()
    {
        ...
        {
            float newScale = 0.01010152f * Mathf.Sqrt(Camera.main.transform.position.y * Camera.main.transform.position.y + Camera.main.transform.position.x * Camera.main.transform.position.x); //localscale factora lready applied in calculation
            foreach (GameObject item in axisDots)
            {
    ...
    GameObject createAxisDot(Vector3 position)
    {
        ...
        axisDot.transform.localScale = new Vector3(1, 0.1f, 1) * (Mathf.Sqrt(Camera.main.transform.position.y * Camera.main.transform.position.y + Camera.main.transform.position.x * Camera.main.transform.position.x) / 40);
        axisDot.transform.position = position;
        axisDot.transform.eulerAngles = new Vector3(90, 0, 0);
